...
class SlugController extends Controller
{
    ...
    public function index($id)
    {
        ...
        if ($menu->slug) {
            ...
            while ($accessDubl) {
                if (!Page::where('slug', $for_slug)->exists()) {
                    $accessDubl = 0;
                } else {
                    $for_slug .= '-1';
                }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!$menu) {
                return redirect()->back()->with('error', 'Menu topilmadi');
            }

            if ($request->page_id && Page::where('id', $request->page_id)->exists()) {
            ...
            \Log::error('Slug store error: ' . $e->getMessage());
